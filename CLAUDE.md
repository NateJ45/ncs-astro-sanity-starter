...
- `npm run scaffold` runs `scripts/scaffold.mjs`, the capability remover. `--list` (or no argument) prints what can go; `--remove <name>` prints the plan; `--remove <name> --write` applies it. It is DRY BY DEFAULT and never touches the Sanity dataset. Run `npm run typegen && npm run build && npm run test:unit` after a removal, which is what it prints. See rule 13 and the marker documentation at the top of the script.
